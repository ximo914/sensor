...
void ShowChargingScr_int(void)
{
    ...
    {
        extern void show_incharging_screen(void);
        show_incharging_screen();
    }
    #else
...
#ifdef __CENON_VOTO_WATCH_APP__
void Voto_LowPower_Off(void)
{
    kal_prompt_trace(MOD_USB," Voto_LowPower_Off");
    MDrv_Soc_Device_Vibrator_Set_OnOff(FALSE);
...
static void mmi_charger_check_low_power_off(S32 battery_level)
{
    ...
    if (!srv_charbat_is_charging() && !srv_bootup_is_booting())
    {
        ...
        #ifdef __CENON_VOTO_WATCH_APP__
        MDrv_Soc_Device_Vibrator_Set_OnOff(TRUE);
        MDrv_Soc_Voto_Function_SendPowerOff();
        StartTimer(TIMER_ID_VOTO_DELAY_POWER_OFF, 2000, Voto_LowPower_Off);
        #else
        ShowLowPowerShutdownScr();
        #endif
    }
...
void BatteryStatusIndication(U8 battery_voltage)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    #if defined(__CENON_A615_YX_PROJECT__)
    U8 power_level = 0;
     switch (battery_voltage)
    {
        ...
        case BATTERY_LOW_WARNING:       /* pass */
            {
                extern void check_if_could_show_low_power(void);
                check_if_could_show_low_power();
            }
            return;
    ...
    {
        extern void set_yx_power_level(U8 v);
        set_yx_power_level(power_level);
        return;
    }
    #endif
    switch (battery_voltage)
...
static void ShowChargingCompleteScr(void)
{
    ...
    #ifdef __CENON_A615_YX_PROJECT__
    {
        extern void show_incharged_screen(void);
        show_incharged_screen();
    }
    #else
